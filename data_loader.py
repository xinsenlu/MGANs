...
def get_loader(root, batch_size, scale_size, data_format, split=None, is_grayscale=False, seed=None):
    ...
    if dataset_name in ['CelebA']:
        queue = tf.image.crop_to_bounding_box(queue, 20, 25, 192, 128)#(width=178, height=218)(crop_to_bounding_box h,w,h,w)
        queue = tf.image.resize_nearest_neighbor(queue, [96, 64])
    else:
        queue = tf.image.resize_nearest_neighbor(queue, [96, 64])
